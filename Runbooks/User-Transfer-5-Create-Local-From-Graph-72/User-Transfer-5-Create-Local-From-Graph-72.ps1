param(
    ...
    [Parameter (Position = 5, HelpMessage = "Enter the User's StartDate")]
    [String] $startDate
)
...
Write-Output "Their Start Date is: $startDate"
...
if ($acctSamNAme.length -gt 20){
    $acctSamName = $acctSamName.substring(0,20)
    Write-Output "The SamAccountName is too long, truncating to 20 characters: $acctSamName"
}
...
Write-Output "Server: $($paramsForRunbookHashTable.Server)"
           ...
           #Standardizes and Sanitizes the User Information 
            ...
            $domainInfo = (Get-ComputerInfo | Select-Object -Property CSDomain).CSDomain

            #If the Runbook is running on the same domain that is havinqg a new user being added, we cannot specify the server. This is in cases where domain trusts are not configured.
            if($domainInfo -eq $paramsFromTicket.Server){
                $managerLocalUser = Get-ADUser -Identity $manager -Credential $myPsCred -ErrorAction SilentlyContinue
                If ($managerLocalUser){
                    Write-Output "Manager is in Local AD, we can continue"
                }
                Else{
                    $manager = $null
                    $newUserOU = $paramsForRunbookHashTable.TargetPath
                    if ($newUserOU.contains('~')){
                        $newUserOU = $newUserOU.replace('~',',')
                    }
                    Write-Output "Manager is not in Local AD, setting to null and using the Target Path: $newUserOU"
                }
                try{
                    $locADUser = Get-ADUser -Identity $acctSamName -Credential $myPsCred
                    Write-Output "$acctSAMName already exists. Modifying."
                        ...
                        -Path $newUserOU `
                        -Surname $lastName `
                        -erroraction Stop -Credential $myPsCred
                        $locADUser = Get-ADUser -Identity $acctSamName -Credential $myPsCred
                    }
                    catch{
                        ...
                        -Path $newUserOU `
                        -Surname $lastName `
                        -SamAccountName $acctSAMName -erroraction Stop -Credential $myPsCred
                        $locADUser = Get-ADUser $acctSamName -Credential $myPsCred
                    }
            }
            #This is for if the running domain is different from the targert domain, in this instance we need to specify the server domain, which is based off the sent parameter.
            else{
                $managerLocalUser = Get-ADUser -Identity $manager -Server $paramsForRunbookHashTable.Server -Credential $myPsCred -ErrorAction SilentlyContinue
                If ($managerLocalUser){
                    Write-Output "Manager is in Local AD, we can continue"
                }
                Else{
...
switch ($usageLoc) {
    "US" {$country = "United States";$countryCode = "840"}
    "BR" {$country = "Brazil";$countryCode = "076"}
    "CA" {$country = "Canada";$countryCode = "124"}
    "ZA" {$country = "South Africa";$countryCode = "710"}
    "MY" {$country = "Malaysia";$countryCode = "458"}
    "IT" {$country = "Italy";$countryCode = "380"}
    "ES" {$country = "Spain";$countryCode = "724"}
    "CN" {$country = "China";$countryCode = "156"}
    "BE" {$country = "Belgium";$countryCode = "056"}
    "AU" {$country = "Australia";$countryCode = "036"}
    "DE" {$country = "Germany";$countryCode = "276"}
    "DK" {$country = "Denmark";$countryCode = "208"}
    "VN" {$country = "Vietnam";$countryCode = "704"}
    "AE" {$country = "United Arab Emirates";$countryCode = "784"}
    "GB" {$country = "United Kingdom";$countryCode = "826"}
    "AT" {$country = "Austria";$countryCode = "040"}
    Default {$null}
}
try{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCzR8s9m6ImlIXq
# h7TA6UELKXzOd9q1Ad0FjCNjkdYPi6CCFAUwggWQMIIDeKADAgECAhAFmxtXno4h
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSI