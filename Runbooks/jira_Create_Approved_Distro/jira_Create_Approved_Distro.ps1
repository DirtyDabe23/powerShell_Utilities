...
try{
    # Read from Azure Key Vault using managed identity
    $connection = Connect-AzAccount -Identity
    $managed_ID = Get-AzKeyVaultSecret -VaultName "KeyVaultName" -Name "ExOManagedIdent" -AsPlainText
    Connect-ExchangeOnline -ManagedIdentity -Organization parentCompanyinc.onmicrosoft.com -ManagedIdentityAccountID $managed_ID
}
catch {
    $errorMessage = $_
    Write-Output $errorMessage
...
$Form = Invoke-RestMethod -Method get -uri "https://parentCompany.atlassian.net/rest/api/2/issue/$jiraTicket" -Headers $jiraHeader
