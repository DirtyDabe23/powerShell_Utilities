...
"@
try {
    Invoke-RestMethod -Uri "https://parentCompany.atlassian.net/rest/api/2/issue/$jiraTicket" -Method Put -Body $jsonPayload -Headers $jiraHeader -erroraction Stop | Out-Null
    $currTime = Get-Date -format "HH:mm"
    Write-Output "[$($currTime)] | [$process] | [$procProcess] Internal Comment Successfully Made with Error Details"
    }
 catch {
...
$partneredparentCompanyLocation =  $form.fields.customfield_10820.value
...
if ($attachment) {
    ...
    ForEach ($user in $csv){
        ...
        $groupName = $partneredparentCompanyLocation , ": UC - Vendor - " , $invitedUserCompanyName, " ", $user.accessLevel -join ""
        If (!(Get-MgGroup -Filter "DisplayName eq  '$groupName'")){
            ...
            $formattedparentCompanyLocation = $($partneredparentCompanyLocation).replace(",","").replace(" ","").replace(".","")
            $groupMailNickName = $formattedparentCompanyLocation , $FormattedCompanyName , $user.accessLevel -join ""
