...
function Rename-parentCompanyUser {
    ...
    Rename-parentCompanyUser -CurrentUserName "TTestUserLast@domain.extension"

    .EXAMPLE
    #The following will update the user to the 'FirstName.LastName' format, without confirmation.
    Rename-parentCompanyUser -currentUserName "testUser@domain.extension" -Auto
    
    .EXAMPLE
    #The following will rename a testUser@domain.extension and set the following properties
    Rename-parentCompanyUser -currentUserName "testUser@domain.extension" -Auto -custom -newUPN "TestName.TestLast-NewLast@domain.extension" -firstName "tName" -lastName "lastName"
    #Properties Changed:
        ...
        try{
            ...
            while ($busySync -or ($waitedTime -lt 50))
            {
                ...
                If (Select-String -InputObject $syncErrorMessage -Pattern "The user name or password is incorrect.")
                {
                    ...
                    $parentCompanyCred = Get-Credential -Message "Enter your parentCompany Admin Account Credentials that can remote into the AADC server.  The notation to use is Test.User@Domain.extension1"
                    $error.Clear()
                }
                else
                {
                    ...
                    $syncResult = Invoke-Command -ComputerName $syncServer -ScriptBlock {Start-AdSyncSyncCycle -PolicyType Delta} -Credential $parentCompanyCred -ErrorAction SilentlyContinue
    ...
    elseIf ($graphOrLocal -eq 1)
    {
        ...
        Else{
            ...
            If ($confirmation.substring(0,1) -eq 'Y'){
                ...
                $parentCompanyRenameJSON = $parentCompanyRenameHashTable | ConvertTo-JSON -Depth 1
                ...
                Invoke-RestMethod -uri $userGraphURI -Method Patch -Body $parentCompanyRenameJSON -Headers $graphAPIHeader -Verbose -Debug
...
try{
    ...
    Rename-parentCompanyUser -currentUserName $originUPN -auto -ErrorAction Stop
