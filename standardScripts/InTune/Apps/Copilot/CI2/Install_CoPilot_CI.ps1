...
$appPath = $currentLocation.Path + "\CoPilotCI.Package.AppInstaller"
...
ForEach ($evapcoProgram in $evapcoPrograms)
{
    ...
    If (Test-path $progName)
    {
        ...
        {
            $Installed = $true
            IF ($evapcoProgram.Version -lt $version)
            {
            Write-Output "Version is Lower, removing."
            Remove-AppXPackage -package $evapcoProgram.PackageFullName -user $evapcoProgram.packageUserinformation.usersecurityID.SID
