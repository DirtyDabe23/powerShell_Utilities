...
$compuDataUsers = Import-CSV -Path "\\parentCompanyusers\departments\Public\Tech-Items\Script Configs\compuDataUsers2.csv"
$errorUsers = @()
...
ForEach ($user in $compuDataUsers)
{
    ...
    try
    {
        ...
        If ($user.'parentCompany Location' -eq 'TT (Location) Users')
        {
        ...
        Else
        {
            ...
            #Create the UPN
            $UPN = $user.'Primary Local AD'.split('@')[0]+"@Domain.extension1"
            ...
            Write-Output "Office: parentCompany East" 
            Write-Output "New User Path: OU=CompuData - External Sage Users - Non-Synching,DC=parentCompany,DC=COM" 
            Write-Output "Surname: $lastName"
            Write-Output "Server: Domain.extension1" 
