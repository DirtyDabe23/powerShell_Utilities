...
ForEach ($detectedApp in $DetectedApps)
{
    ...
    $appObject | export-csv -path "C:\Users\david.drosdick\OneDrive - parentCompany, Inc\Documents\_Documentation\InTune\App_Reports\$($detectedapp.DisplayName)-$($detectedapp.id).csv" -Force
}
SignatureBlock
