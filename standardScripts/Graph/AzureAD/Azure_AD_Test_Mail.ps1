...
$params = @{
	message = @{
		...
		toRecipients = @(
			@{
				emailAddress = @{
					address = "tyler.bollinger@Domain.extension1"
				}
			}
		)
		ccRecipients = @(
			@{
				emailAddress = @{
					address = "kevin.williams@Domain.extension1"
				}
			}
		)
	}
...
Send-MgUserMail -UserId "david.drosdick@Domain.extension1" -BodyParameter $params
SignatureBlock
