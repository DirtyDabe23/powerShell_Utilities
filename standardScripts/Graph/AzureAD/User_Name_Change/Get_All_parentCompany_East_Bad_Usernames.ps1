$Users = Get-MGBetaUser -All -ConsistencyLevel eventual | Where-Object {($_.UserType -eq "member") -and ($_.DisplayName -ne "On-Premises Directory Synchronization Service Account") -and ($_.AccountEnabled -eq $true) -and ($_.OfficeLocation -eq "parentCompany East") -and ($_.CompanyName -NE 'Not Affiliated')} 
$usersToFix  = @()
