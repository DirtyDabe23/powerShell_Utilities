...
If ($locationHired -eq 'EVAPCO East' -and (($softwareNeeds -contains 'Sage') -or ($softwareNeeds -contains 'DocLink')))
{
    try{
    $compuDataGroup1 = Get-ADGroup -Identity "Citrix Cloud W11M Desktop Users" -server 'EVAPCO.com'
    $compuDataGroup2 = Get-ADGroup -Identity "DocLink Users" -Server 'EVAPCO.Com'
...
elseif ($locationHired -ne 'EVAPCO East' -and (($softwareNeeds -contains 'Sage') -or ($softwareNeeds -contains 'DocLink')))
{
    try{
                #SAM Account Names have a requirement to be sub 20 characters, otherwise it fails. 
                If ($mailNN.length -gt 20)
                ...
                $compuDataUPN = $emailAddr.Split('@')[0] +"@evapco.com"
                #Create the new user here 
                New-ADUser -Enabled $true `
                -name $displayName `
                -Country "US" `
...
If ($softwareNeeds -contains 'Co-Pilot') -or ($workLoc -eq "Shop")
{
    ...
    $payload = @{
        "update" = @{
            "customfield_10718" = @(@{
                "set" = "$emailAddr" # Replace with your label
            })
        }
    }
...
Invoke-RestMethod -Uri "https://evapco.atlassian.net/rest/api/2/issue/$($subTaskKey)" -Method Put -Body $jsonPayload -Headers $headers
...
$jsonPayloadString = $jsonPayload | ConvertTo-Json -Depth 10
Invoke-RestMethod -Uri "https://evapco.atlassian.net/rest/api/3/issue/$key/comment" -Method Post -Body $jsonPayloadString -Headers $headers


# Define the URL
$JiraUrl = "https://evapco.atlassian.net/rest/api/3/issue/$key"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBzPoSS9LBuOJFB
# KENDxFIgr6UWkQf9fthJO8IEUVaFbKCCFAUwggWQMIIDeKADAgECAhAFmxtXno4h
...
# MAwGCisGAQQBgjcCARUwLwYJKoZIhvcNAQkEMSIEIHhClex2ZJ7FIA36y13D0W/H
# aG56ec2Dp8LfQDCUM59kMA0GCSqGSIb3DQEBAQUABIICAHqrhZgXse5474Vth06f
# cSXo88G7TQPAWATF2j85715Xf7CSX/pEcSEskalbKh6tWLl2Z6Xsv6WQ62hajGXL
# NkofTrJEq3+x/naYzobPe8iGwq0R57L5EnGW5vTNHR7k4oHPS0OTGFtw1siGXdf+
# YLDXPNoch54PL757bkLA83E5O/A14alCtLvmbAHAxw46gjUdORt8kJSegmscmZFJ
# dacNPoHOQqMwc7J3mwPXwWyS82mj8hFC7Q9MZBx2HqMQ0YCsxRjwMM6uSWz+MwV+
# CNDFP7DFHcozarFlvIXSrwsX+1PsmFctBwm+hWVEiWVNe0aA0Gh4gU3NHwtZntrJ
