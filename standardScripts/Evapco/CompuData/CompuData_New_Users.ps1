...
$compuDataUsers = Import-CSV -Path "\\evapcousers\departments\Public\Tech-Items\Script Configs\compuDataUsers2.csv"
$errorUsers = @()
...
ForEach ($user in $compuDataUsers)
{
    ...
    try
    {
        ...
        If ($user.'Evapco Location' -eq 'TT (TaneyTown) Users')
        {
        ...
        Else
        {
            ...
            #Create the UPN
            $UPN = $user.'Primary Local AD'.split('@')[0]+"@evapco.com"
            ...
            Write-Output "Office: EVAPCO East" 
            Write-Output "New User Path: OU=CompuData - External Sage Users - Non-Synching,DC=EVAPCO,DC=COM" 
            Write-Output "Surname: $lastName"
            Write-Output "Server: EVAPCO.COM" 
