    ...
    If ($graphOrLocal -eq 2)
    {
        ...
        $upnSuffix ="@" +$usertoModify.userPrincipalName.Split('@')[1]
                ...
                Else{
                    ...
                    try {
                        Invoke-Command -ComputerName PREFIX-VS-AADC01 -ScriptBlock {Start-AdSyncSyncCycle -PolicyType Delta} -erroraction Stop
                    }
                    catch{
                        $busySync = $true
                        while ($busySync)
                        {
                            ...
                            $syncResult = Invoke-Command -ComputerName PREFIX-VS-AADC01 -ScriptBlock {Start-AdSyncSyncCycle -PolicyType Delta} -ErrorAction SilentlyContinue
