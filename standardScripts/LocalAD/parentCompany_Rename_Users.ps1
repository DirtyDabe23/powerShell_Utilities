    ...
    If ($graphOrLocal -eq 2)
    {
        ...
        $upnSuffix ="@" +$usertoModify.userPrincipalName.Split('@')[1]
                ...
                Else{
                    ...
                    $userDataPath = "\\parentCompanyusers\users\$oldSAM"
                    $newUserPath = "\\parentCompanyusers\users\$acctSAMName"
                    ...
                    try {
                        Invoke-Command -ComputerName ServerAADC01 -ScriptBlock {Start-AdSyncSyncCycle -PolicyType Delta} -erroraction Stop
                    }
                    catch{
                        $busySync = $true
                        while ($busySync)
                        {
                            ...
                            $syncResult = Invoke-Command -ComputerName ServerAADC01 -ScriptBlock {Start-AdSyncSyncCycle -PolicyType Delta} -ErrorAction SilentlyContinue
