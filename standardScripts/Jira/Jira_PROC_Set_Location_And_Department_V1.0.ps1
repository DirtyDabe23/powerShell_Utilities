...
try {
    ...
    $jiraRetrSecret = Get-AzKeyVaultSecret -VaultName "KeyVaultName" -Name "jiraAPIKey" -AsPlainText | Out-Null
}
catch {
    $errorMessage = $_
...
$Issue = Invoke-RestMethod -Method get -uri "https://parentCompany.atlassian.net/rest/api/2/issue/$ghdKey" -Headers $jiraHeaders
...
{
    If ($officeLocation -eq 'parentCompany East')
    {
        ...
        if ($department -eq 'parentCompanyld')
        {
...
switch ($departmentID) {
   ...
   Invoke-RestMethod -Uri "https://parentCompany.atlassian.net/rest/api/2/issue/$($procKey)" -Method Put -Body $jsonPayload -Headers $jiraHeaders
