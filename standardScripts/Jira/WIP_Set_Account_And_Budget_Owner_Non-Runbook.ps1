...
$Issue = Invoke-RestMethod -Method get -uri "https://evapco.atlassian.net/rest/api/2/issue/$ghdKey" -Headers $headers
...
If ($null -eq $accountNumber)
{
    ...
    If ($officeLocation -eq 'EVAPCO Midwest')
    {
        ...
        $budgetOwner = "Michael Giannavola"
    }
    If ($officeLocation -eq 'EVAPCO Iowa')
    {
        $accountNumber = "6680-06-00"
...
Else
{
   ...
   else 
   {
        switch ($accountNumber) {
            ...
            "6680-02-00"{$BudgetOwner = "Michael Giannavola"}
            "6680-02-20"{$BudgetOwner = "Michael Giannavola"}
            ...
            "1355-01-00"{$BudgetOwner = "Brian Walker"}
...
Invoke-RestMethod -Uri "https://evapco.atlassian.net/rest/api/2/issue/$($procKey)" -Method Put -Body $jsonPayload -Headers $headers
