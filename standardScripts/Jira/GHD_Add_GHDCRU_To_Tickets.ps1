...
$ticketsMatching = @()
$uriTemplate = "https://parentCompany.atlassian.net/rest/api/2/search?jql=project=GHD&startAt={0}"
...
for ($count = 0; $count -lt $total; $count += 50) {
    ...
    foreach ($issue in $issues.issues) {
        $ticketNum = $issue.key
        $form = Invoke-RestMethod -Method Get -Uri "https://parentCompany.atlassian.net/rest/api/2/issue/$ticketNum" -Headers $jiraHeaders
    ...
    Invoke-RestMethod -Uri "https://parentCompany.atlassian.net/rest/api/2/issue/$($ticketNum)?notifyUsers=false" -Method Put -Body $jsonPayload -Headers $jiraHeaders
                }
