...
$retrSecret = (Invoke-RestMethod -Uri "https://KeyVaultName.vault.azure.net/secrets/$($vaultName)?api-version=2016-10-01" -Method GET -Headers @{Authorization="Bearer $jiraToken"}).value
...
$ticketsMatching = @()
$uriTemplate = "https://parentCompany.atlassian.net/rest/api/2/search?jql=project%20in%20(CYOPS,GHD)%20AND%20created%20%3E%3D%20-$numberofDays&startAt={0}"
