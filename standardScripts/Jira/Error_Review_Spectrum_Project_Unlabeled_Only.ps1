...
$pageCount = 1
# Import CSV
$errorsToReview = Import-Csv -Path "\\evapcousers\departments\Public\Tech-Items\Script Configs\devErrors.csv"

# Jira API Setup
$encodedText = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes("david.drosdick@evapco.com:$jiraRetrSecret"))
$headers = @{
...
$ticketsMatching = @()
$uriTemplate = "https://evapco.atlassian.net/rest/api/2/search?jql=project%3Dspec%20AND%20labels%20%3D%20EMPTY&startAt={0}"
...
for ($count = 0; $count -lt $total; $count += 50) {
    ...
    Invoke-RestMethod -Uri "https://evapco.atlassian.net/rest/api/2/issue/$($ticketNum)?notifyUsers=false" -Method Put -Body $jsonPayload -Headers $headers
                    break
                }
            }
        }
...
$exportPath = "\\evapcousers\departments\public\Tech-Items\scriptLogs\error_report.csv"
