...
Try
{
    ...
    Test-Path "\\evapcousers\departments\public\tech-items\misc\profiles.dlps" -ErrorAction Stop
    Copy-Item -Path "\\evapcousers\departments\public\tech-items\misc\profiles.dlps" -Destination "$env:AppData\altefc products, inc\doc-link\profiles.dlps" -Force -ErrorAction Stop
}
Catch
{
    $errorLog += [PSCustomObject]@{
